...
# ISJ project 5 by Aleksandr Verevkin (xverev00) VUT FIT 2020/2021
...
# Solution:
    ...
    for number in index:
        try:
            ...
            print("Ignoring index {0} - {1}".format(number, error))
        else:
            answers = []
            for answer in zip(altcodes, new[1]):
                answers.append(answer[0] + ': ' + answer[1])
            quiz.append((new[0], answers))
    return quiz
